<#
.SYNOPSIS
  Advanced command history management for Fin.
#>

function ConvertTo-NormalizedArgs {
    [CmdletBinding()]
    ...
    foreach ($arg in $InputArgs) {
        if ($arg -match '^-(\d+)$') { $numbers += [int]$arg }
        elseif ($arg -match '^\d+$') { $numbers += [int]$arg }
        elseif ($arg -match '^-(\w+)$') {
            $flag = $Matches[1]
            if ($flag -notmatch '^\d+$') { $flags += "-$flag" } else { $numbers += [int]$arg }
        }
        elseif ($arg -match '^--(\w+)$') { $flags += $arg }
        elseif ($arg -match '^/(\w+)$') { $flags += "-$($Matches[1])" }
...
function Get-HistoryItemRelative {
    [CmdletBinding()]
    param([int]$RelativeIndex,[switch]$Full)

    $history = if ($Full) { Get-GlobalHistory } else { Get-SessionHistory }
    $count = $history.Count
    if ($count -eq 0) { return "" }

    $targetIndex = if ($RelativeIndex -eq 0) {
        $count - 1
    } elseif ($RelativeIndex -lt 0) {
        $count + $RelativeIndex
    } else {
        $RelativeIndex - 1
    }

    if ($targetIndex -ge 0 -and $targetIndex -lt $count) {
        return $history[$targetIndex]
    } else {
        Write-Warning "History index out of range"
        return ""
    }
}

function Invoke-HistoryUnified {
    [CmdletBinding()]
    param([string[]]$RemainingArgs)

    $normalized = ConvertTo-NormalizedArgs $RemainingArgs

    # Full history mode (-f or --full)
    if ($normalized.Flags -contains '-f' -or $normalized.Flags -contains '--full') {
        $history = Get-GlobalHistory
        for ($i = 0; $i -lt $history.Count; $i++) {
            Write-Output ("{0,4} {1}" -f ($i + 1), $history[$i])
        }
        return
    }

    # Single numeric argument only
    if ($normalized.Numbers.Count -eq 1 -and $normalized.Flags.Count -eq 0 -and $normalized.PassThrough.Count -eq 0) {
        $n = $normalized.Numbers[0]
        $cmd = Get-HistoryItemRelative -RelativeIndex $n
        if ($cmd) { Write-Output $cmd }
        return
    }

    # Default: native Get-History
    Get-History
}

# Override built-in history alias cleanly
if (Get-Alias history -ErrorAction SilentlyContinue) {
    Remove-Item Alias:history -Force -ErrorAction SilentlyContinue
}
Set-Alias history Invoke-HistoryUnified -Option AllScope -Force


# SIG # Begin signature block
# MIIb5gYJKoZIhvcNAQcCoIIb1zCCG9MCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUP/lOqnnxi9utVLSzYoj3lllQ
